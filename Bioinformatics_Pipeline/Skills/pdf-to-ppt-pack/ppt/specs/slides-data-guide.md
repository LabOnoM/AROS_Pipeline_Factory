...
| `elements` | array | Yes | Array of content elements |

## Component Types

### 1. comparison - Comparison Card

Left and right two-column comparison layout.
...
Simulates a terminal/command line display.
...
Displays content where old assumptions are broken, supporting a strikethrough reveal effect.
...
- `badge`: Phase label
- `badgeCurrent`: Whether to mark as the current phase
- `icon`: Icon character
- `iconFilled`: Whether to fill the icon
- `title`: Title
- `desc`: Description
- `case`: Case study/Example description
...
Displays core value points, supporting sequential activation.
...
```javascript
{
    ...
    items: [
        ...
        { type: "wrong", text: "Also things not to do" },
...
// Element level - Single element animation
{
    step: 1,
    type: "quote",
...
1. **Special Unicode Symbols**
   - Forbidden: emoji (🚀📝), special geometric symbols (✦◈◆◇●○◐◑)
   - Alternative: Use ASCII characters (`*`, `+`, `-`, `#`, `@`, `o`, `O`, `=`)

2. **Chinese Quotation Marks**
   - Forbidden: 「」『』""''
   - Alternative: Use standard quotes or omit them

3. **Single Quotes within Strings**
   - Forbidden: `"Entering 'Doing' Year One"` (Single quotes will be converted to double quotes, breaking the JSON)
   - Alternative: `"Entering Doers Year One"` or use other expressions

4. **Comments inside Arrays**
   - Forbidden: Using `// comments` inside the SLIDES array
...
2. **Check PPTX Parsing**: If PPTX conversion fails, check if forbidden characters were used.
