...
Adaptyv is a cloud laboratory platform for automated protein testing and validation. Submit protein sequences via API (or web UI), track experiment status, and download results (typically delivered in ~21 days).
...
- If a file is produced, prefer a deterministic output name such as `adaptyv_result.json` unless the skill documentation defines a better convention.
...
```text
Result file: adaptyv_result.json
