...
```bash
cd "20260316/scientific-skills/Others/short-video-script-generator"
...
1. Branch based on script type
2. Generate script outline for the specific type
3. Refine script details
4. Output final script
...
- If a file is produced, prefer a deterministic output name such as `short_video_script_generator_result.md` unless the skill documentation defines a better convention.
...
```text
Result file: short_video_script_generator_result.md
...
## Integration with Production Pipeline (Merged from Benchflow)

- After script generation, direct the Swarm to use `ffmpeg-core`, `text-to-speech`, and `ffmpeg-video-editing` to produce the final multimedia asset.
- Scripts must explicitly mark visual transitions to map to `ffmpeg-video-filters`.
