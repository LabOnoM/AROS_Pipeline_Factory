---
name: scanpy
description: Standard single-cell RNA-seq analysis pipeline for QC, normalization, dimensionality reduction, clustering, differential expression, and visualization. Best for exploratory scRNA-seq using established workflows. For deep learning, use scvi-tools; for data format, use anndata.
...
Scanpy is a scalable Python toolkit for analyzing single-cell RNA-seq data, built on AnnData. This skill performs complete single-cell workflows including quality control, normalization, dimensionality reduction, clustering, marker gene identification, visualization, and trajectory analysis.
...
- Identifying cell clusters and marker genes.
- Annotating cell types based on gene expression.
- Conducting trajectory inference or pseudotime analysis.
- Generating publication-quality single-cell plots.
- `scripts/qc_analysis.py` is the most direct path to complete the request.
- You need the `scanpy` package behavior rather than a generic answer.

## Key Features

- Standard single-cell RNA-seq analysis pipeline.
- Packaged executable: `scripts/qc_analysis.py`.
- Reference material in `references/` for task-specific guidance.
- Reusable asset: `assets/analysis_template.py`.
- Structured execution for consistent and reviewable outputs.

## Dependencies

- `Python`: 3.10+
- `Third-party packages`: Dependencies are not explicitly version-pinned in this skill package. Add pinned versions if this skill needs stricter environment control.
...
1. Confirm input/output paths and config values.
2. Edit the in-file `CONFIG` block or documented parameters if the script uses fixed settings.
3. Run `python scripts/qc_analysis.py` with validated inputs.
4. Review output and return the final artifact, noting any assumptions.
