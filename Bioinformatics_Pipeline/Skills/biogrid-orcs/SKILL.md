...
- Packaged executable path(s): `scripts/biogrid_client.py`.
...
3. Run `python scripts/biogrid_client.py` with the validated inputs.
...
- Primary implementation surface: `scripts/biogrid_client.py`.
...
The skill provides a Python script `scripts/biogrid_client.py` that maps to the API endpoints.
...
- If a file is produced, prefer a deterministic output name such as `biogrid_orcs_result.md` unless the skill documentation defines a better convention.
