---
name: kegg-database
description: Direct REST API access to KEGG (academic use only). Pathway analysis, gene-pathway mapping, metabolic pathways, drug interactions, ID conversion. Use for precise HTTP/REST control or KEGG-specific ID mapping.
license: MIT
metadata:
...
**Important**: KEGG API is made available only for academic use by academic users. Non-academic/commercial use may require a separate KEGG license.

## When to Use This Skill

This skill should be used when:

- Querying pathways, genes, compounds, enzymes, diseases, and drugs across multiple organisms using KEGG's REST API.
- Performing gene ↔ pathway mapping (e.g., building inputs for pathway enrichment analysis).
- Needing cross-references between KEGG databases (e.g., pathway → genes, gene → KO, pathway → compounds).
- Converting identifiers between KEGG and external databases (e.g., KEGG gene → NCBI Gene ID / UniProt; KEGG compound → PubChem).
- Performing drug–drug interaction (DDI) lookups for KEGG drug IDs.

## Key Features

- Full coverage of core KEGG REST operations via Python helper functions in `scripts/kegg_api.py`:
...
- Comprehensive reference documentation in `references/kegg_reference.md` with detailed API specifications, database lists, and organism codes.
...
Retrieve metadata and statistics about KEGG databases or specific organisms.
...
**Output formats**: `aaseq` (protein FASTA), `ntseq` (nucleotide FASTA), `mol` (MOL format), `kcf` (KCF format), `image` (PNG), `kgml` (XML), `json` (pathway JSON). **Important**: Image, KGML, and JSON formats allow only one entry at a time.
...
- **Entry limits**: Maximum 10 entries per operation, except `image/kgml/json` which allow only 1 entry. Chunk lists into batches of ≤10 IDs and enforce single-entry calls for `image/kgml/json`.
