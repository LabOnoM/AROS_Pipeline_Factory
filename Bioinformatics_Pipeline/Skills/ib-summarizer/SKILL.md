...
- Use this skill when the task is to Summarize core safety information from Investigator's Brochures for clinical.
- Use this skill for evidence insight tasks that require explicit assumptions, bounded scope, and a reproducible output format.
- Use this skill when you need a documented fallback path for missing inputs, execution errors, or partial evidence.

## Key Features

- Scope-focused workflow aligned to: Summarize core safety information from Investigator's Brochures for clinical.
...
```bash
cd "20260318/scientific-skills/Evidence Insight/ib-summarizer"
...
This skill accepts requests that match the documented purpose of `ib-summarizer` and include enough context to complete the workflow safely.

Do not continue the workflow when the request is out of scope, missing a critical input, or would require unsupported assumptions. Instead respond:

> `ib-summarizer` only handles its documented workflow. Please provide the missing required inputs or switch to a more suitable skill.
