...
## Policy Mandates: Results & Figure Presentation
This skill enforces the AROS 'Results & Figure Presentation' policy. The guiding principle of this policy is to: **Present statistical results and generated figures clearly, logically, and interpretively within the manuscript.**

To uphold this standard, the following non-negotiable error prevention rules are enforced:

1.  **Strict Numerical and Entity Matching:** The consistency check MUST enforce exact matching for all numerical values (e.g., measurements, n-values, p-values), statistical tests, and named entities (e.g., genes, proteins, experimental group names) between the main Results text and the figure legends. Any mismatch must be flagged as a high-priority issue.

2.  **Mandatory Citation Check:** Every figure and sub-panel described in a legend (e.g., "Figure 1A", "Figure 3B") MUST be explicitly cited in the main Results section narrative. A missing citation constitutes a high-priority 'Missing Reference' issue.

3.  **Scope Limitation Declaration:** The analysis is strictly limited to the provided text-based input (Markdown conversion of a document). The skill MUST NOT infer or validate visual content from figure images. This limitation must be explicitly stated in the header of the generated `consistency_report.md`.
