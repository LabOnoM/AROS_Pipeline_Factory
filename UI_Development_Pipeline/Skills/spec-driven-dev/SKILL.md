...
original-source: addyosmani/agent-skills/spec-driven-development
---
...
```
  SPEC ──▶ PLAN ──▶ BDD/TDD ──▶ BUILD ──▶ TEST ──▶ REVIEW ──▶ SHIP
   │        │      (Red State)    │        │        │          │
   ▼        ▼          ▼          ▼        ▼        ▼          ▼
 What &   Tasks &   Features +  Make     Prove    Five-axis  Deploy,
  why     order    failing      tests    refactor quality    observe,
                   tests        pass     safe     gate       roll back
   │        │          │          │        │        │          │
   └────────┴──────────┴──────────┴────────┴────────┴──────────┘
...
## Phase 2.5: BDD & TDD (Red State)

**Goal.** Translate the approved spec into executable behavioral constraints
BEFORE any implementation code is written. This phase is owned exclusively by
the `qa_engineer` agent persona. The `code_generator` MUST NOT participate.

**Inputs.** Approved SPEC.md, approved PLAN.md, existing test infrastructure.

**Steps.**

1. Read the SPEC.md and identify all behavioral requirements.
2. Write Gherkin `.feature` files describing each behavior using
   Given/When/Then syntax. Save to the project's `.wiki/features/` directory.
3. Write `pytest-bdd` step definitions in `tests/step_defs/` that import the
   `.feature` scenarios. These step definitions MUST compile but MUST FAIL
   (Red State) because no implementation exists yet.
4. Write supplementary `pytest` unit tests for edge cases and error paths
   not covered by BDD scenarios.
5. Run `pytest` and confirm ALL new tests FAIL. If any test passes without
   implementation, the test is likely testing the wrong thing — rewrite it.
6. Save a test execution log showing the Red State output.

**Exit criteria.**

- [ ] Every behavioral requirement in SPEC.md has a corresponding `.feature` scenario
- [ ] Step definitions exist and compile cleanly
- [ ] `pytest` runs and ALL new tests FAIL (Red State confirmed)
- [ ] Red State log saved as evidence

**Common Rationalizations**

| Excuse | Reality |
|---|---|
| "I'll write features after I see the code" | That's testing, not specification. The whole point is to constrain the code BEFORE it exists. |
| "BDD is too verbose for this" | A 5-line Given/When/Then is cheaper than debugging an agent that built the wrong thing. |
| "The SPEC is clear enough" | SPECs describe intent. Features describe executable behavior. They are complementary, not redundant. |
...
6. **Red-Green Gate.** The `code_generator` agent MUST NOT begin implementation
   until the `qa_engineer` has confirmed Red State (all tests failing). The
   code_generator's sole objective is to make the failing tests pass with the
...
8. After Green State is achieved, run `pytest` one final time. If all tests
   pass, the code_generator MAY refactor for readability and performance.
   After each refactor step, run `pytest` again — tests MUST remain green.
...
## Specification Generation Template (Merged from Osmani)

When generating a specification, use this 6-area template:
1. **Objective:** What is the goal?
2. **Commands:** How is the code built, tested, and run?
3. **Structure:** What is the architecture and file layout?
4. **Code Style:** What are the linting and formatting rules?
5. **Testing:** What is the testing strategy (unit, integration, e2e)?
6. **Boundaries:** Use a 3-tier boundary system (`Always do` / `Ask first` / `Never do`).

**Verification Checklist:** Always verify the spec covers these 6 areas before proceeding.
