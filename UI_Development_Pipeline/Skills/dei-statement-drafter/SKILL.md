...
- Use this skill when the task is to Draft Diversity, Equity, and Inclusion statements for academic applications.
- Use this skill for academic writing tasks that require explicit assumptions, bounded scope, and a reproducible output format.
- Use this skill when you need a documented fallback path for missing inputs, execution errors, or partial evidence.

## Key Features

- Scope-focused workflow aligned to: Draft Diversity, Equity, and Inclusion statements for academic applications.
...
```bash
cd "20260318/scientific-skills/Academic Writing/dei-statement-drafter"
...
This skill accepts requests that match the documented purpose of `dei-statement-drafter` and include enough context to complete the workflow safely.

Do not continue the workflow when the request is out of scope, missing a critical input, or would require unsupported assumptions. Instead respond:

> `dei-statement-drafter` only handles its documented workflow. Please provide the missing required inputs or switch to a more suitable skill.
