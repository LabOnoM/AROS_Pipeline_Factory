...
### GEPA Error Prevention Rule: File Presence and Accessibility

After confirming the scope is correct, you **MUST** validate that all 6 mandatory input files exist and are accessible before calling the assembly script. This prevents runtime errors from missing or unreadable files.

**Action:**
- Use a shell command to check for the existence of each of the 6 input files.
- If any file is missing or not readable, you MUST stop and report the issue to the user.

**Example Validation Command:**
```bash
# Before running the python script, check all 6 files.
# This example uses a simple loop; adapt as needed.
files=("A.png" "B.png" "C.png" "D.png" "E.png" "F.png")
for file in "${files[@]}"; do
  if [ ! -f "$file" ]; then
    echo "Validation failed: Input file not found or is not a regular file: $file"
    exit 1
  fi
done
...
**Refusal Template for Missing Files:**
> "Input validation failed: The following mandatory input files were not found or are inaccessible: `[list missing file(s)]`. Please ensure all 6 panel images are provided and accessible in the workspace."
...
1. **Validate input** — confirm scope, presence of exactly 6 panels, and file accessibility before any processing. Do not generate any output before this check.
...
- If any input file is missing or inaccessible (as per the GEPA rule), report the missing files and stop.
...
Risks          : [aspect ratio mismatch, resolution loss]
───────────────────────────────────────
```
