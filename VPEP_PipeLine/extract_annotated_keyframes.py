"""
VPEP (Video-to-Protocol Extraction Pipeline) - Keyframe Extractor & Annotator
=============================================================================

Description:
    This script handles Stage 2 of the VPEP pipeline. It extracts frames at 1fps within each procedural
    timeline step, runs MediaPipe Hands to filter for hand-active frames, and queries Gemini 2.5/3.5 Flash
    to select the best keyframe and identify bounding boxes for key reagents/objects. It draws EGL/MediaPipe
    skeletons and bounding boxes onto the keyframe images.

Key Capabilities:
    - Optimized 1fps frame extraction using OpenCV direct seeks for minimal CPU decoding overhead.
    - MediaPipe Hands skeletal overlay.
    - Gemini multimodal queries for object detection and bounding box predictions [ymin, xmin, ymax, xmax].
    - Cross-platform title drawing and OpenCV image saves.

Cross-Platform Compatibility:
    - Independent of platform-specific display systems (runs headless or via EGL graphics pipeline).
    - Platform-agnostic path separation.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Extract annotated keyframes with MediaPipe and Gemini.")
    ...
    parser.add_argument("--static_image_mode", type=lambda x: (str(x).lower() == 'true'), default=True, help="MediaPipe static image mode.")
    parser.add_argument("--max_num_hands", type=int, default=4, help="MediaPipe max num hands.")
    parser.add_argument("--min_detection_confidence", type=float, default=0.65, help="MediaPipe min detection confidence.")
    parser.add_argument("--min_tracking_confidence", type=float, default=0.65, help="MediaPipe min tracking confidence.")
    parser.add_argument("--title_position", default="lower_left", choices=["lower_left", "upper_right"], help="Position to draw the title text.")
