---
name: matchms
description: Process and compare mass spectrometry (MS/MS) spectra for metabolomics, enabling reproducible spectral filtering and similarity scoring.
...
Matchms is an open-source Python library for mass spectrometry data processing and analysis. It facilitates importing spectra, standardizing metadata, filtering peaks, calculating spectral similarities, and building reproducible analytical workflows. It is particularly useful for metabolite identification, spectral matching, and library searching.

## When to Use

- When you need to import, process, clean, and compare mass spectrometry (MS/MS) spectra.
- When you require reproducible spectral filtering and similarity scoring for metabolomics workflows.
- When a data analytics task benefits from a packaged method for consistent results.
- When you need `matchms` package behavior rather than a generic answer.
...
Apply filters to standardize metadata and refine peak data:
...
- **CosineGreedy/CosineHungarian**: Peak-based cosine similarity with different matching algorithms. Greedy matching selects best available peak matches iteratively, while Hungarian matching solves an assignment problem to maximize the total match score.
- **ModifiedCosine**: Cosine similarity accounting for precursor mass differences, improving matching for related compounds/adducts. Requires precursor m/z metadata.
- **NeutralLossesCosine**: Similarity based on neutral loss patterns.
- **FingerprintSimilarity**: Molecular structure similarity using fingerprints (requires structure annotations or reliable fingerprint computation).
...
Prefer explicit, ordered filter chains and pinned dependency versions for workflow reproducibility. Store configuration (tolerances, normalization choices, filters used) alongside results for traceability. See `references/workflows.md` for pipeline organization guidance.
...
Matchms operates on `Spectrum` objects containing peak m/z and intensity arrays plus metadata (e.g., precursor m/z, charge, compound name/identifier).
...
Typical pipelines apply filters to standardize/repair metadata fields, remove noise peaks (e.g., by intensity threshold or m/z window rules), and normalize intensities (commonly to a maximum of 1.0 or to unit norm).
