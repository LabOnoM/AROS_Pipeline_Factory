...
Using NLP to analyze a summary of NIH/NSF/Horizon Europe's winning projects in the past 6 months,
Predicting shifts in funding agency preferences over the next 3-5 years.
...
class TextProcessor:
    """Text preprocessing and analysis classes"""
...
class TopicModeler:
    ...
    def fit(self, documents: List[str]) -> List[Dict]:
        """Simple topic modeling based on word frequency and co-occurrence analysis
        In actual applications, it can be replaced by LDA or BERTopic"""
    ...
    def get_emerging_topics(self, historical_data: List[str], 
        ...
        for term, recent_count in recent_freq.most_common(100):
            ...
            if growth_rate > 0.3 and recent_count >= 5:  # Growth rate exceeds 30%
                emerging.append({
                    'term': term,
...
class FundingTrendForecaster:
    """Funding Trend Forecast Main Category"""
    ...
    def generate_mock_data(self, months: int = 6, 
        ...
        """Generate simulated data sets for demonstration
        In actual applications, real data should be obtained from API"""
    ...
    def analyze_trends(self) -> Dict:
        ...
        # Convert a Period to a string for JSON serialization
        monthly_trends_dict = {}
        for period, row in monthly_trends.iterrows():
            monthly_trends_dict[str(period)] = row.to_dict()
        # Identify the fastest growing areas
        domain_growth = {}
    ...
    def predict_trends(self, years: int = 5) -> Dict:
        ...
        # High growth area forecasts
        high_growth_domains = [d for d, g in domain_growth.items() if g > 0.2]
        
        for year_offset in range(1, years + 1):
            year = base_year + year_offset
            confidence = max(0.5, 0.9 - (year_offset * 0.05))  # Confidence decreases over time
            
            # Predict hot topics based on growth rate
            predicted_topics = []
    ...
    def _predict_emerging_areas(self, year_offset: int) -> List[str]:
        """Anticipating emerging research areas"""
        emerging_pools = {
...
def main():
    ...
    # Generate simulation data (actually should be obtained from API)
