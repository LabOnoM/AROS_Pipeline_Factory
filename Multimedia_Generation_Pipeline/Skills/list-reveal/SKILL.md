---
name: list-reveal
description: "# Anchor: list-reveal（列举型逐个揭示） > ⚠️ **这是结构示意，不是抄袭模板**。先走 [`../../CHAPTER-CRAFT.md`](../../CHAPTER-CRAFT.md) > Part 0 五问。本 anchor 给的是'列举型章节的结构骨架'（单网格 N 槽位 + > 每 step 只填一个槽位 + 位置不重排）——保留这个**结构**，**按本项目的 > 主题 + 内容换动作选型**。倒过来照抄 = [`../../CHAPTER-CRA..."
---
