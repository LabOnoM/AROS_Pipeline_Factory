---
name: tts-providers
description: "# TTS Providers `synthesize-audio.sh` 是 provider-agnostic 的 runner —— 它自己不知道 怎么调任何 TTS，只知道循环 `audio-segments.json`、跳过已存在文件、 打印进度。 **每个 provider 是这个目录下的一个 `.sh` 文件**，定义一个 `tts_synthesize` 函数（必需），以及可选的 `tts_check` 和 `tts_install_help`。runner 根据 `P..."
---
